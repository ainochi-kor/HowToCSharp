...
namespace ChattingSystem_Server
{
    public partial class ServerForm : Form
    {
            ...
            try
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        SendMessageAll(IpPoint.Address + ":" + IpPoint.Port + " 님이 연결되었습니다.", channel, true);
        ...
        {
            try
            {   // 20200820 클라이언트로 부터 받는 메시지 그대로 출력 및 전송 수정
                DisplayText(message);
        ...
        {   
            try
            {   // 20200820 클라이언트 해제시 Client 해제와 동시에 메시지 처리로 수정 

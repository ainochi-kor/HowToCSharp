...
namespace ChattingSystem_Server
{
    public class ServerEvent
    {
        ...
        {
            //log4net.ILog logger = log4net.LogManager.GetLogger(typeof(ServerEvent));
            log4net.Config.XmlConfigurator.Configure(new System.IO.FileInfo("log_ChattingSystem.xml"));
            logger.Info("Send: " + sendData);
        }

        public void ReceiveServerLog(string ReceivedData)
        {
            //log4net.ILog logger = log4net.LogManager.GetLogger(typeof(ServerEvent));
            log4net.Config.XmlConfigurator.Configure(new System.IO.FileInfo("log_ChattingSystem.xml"));
            logger.Info("Recv: " + ReceivedData);
        }

        public void ErrorLog(string className, string error)
        {
            log4net.Config.XmlConfigurator.Configure(new System.IO.FileInfo("log_ChattingSystem.xml"));
            logger.Error("ClassName : '" + className + "' - " + error);
        }
